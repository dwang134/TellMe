...
const History = ({transactions}) => {
  ...
  return (
    ...
    <div className= 'grid gap-4'>
    {
      ...
      <button className='w-3/12 px-3' onClick= {handleDelete}><BiTrash style= {{color: transaction.color, fontSize: 'large' }}></BiTrash></button>
      <div className= 'w-full h-fit bg-blue-200 text-center'>{transaction.desc}</div>
      <div className= 'w-3/12 h-8 border-r-8 rounded' style= {{borderColor: transaction.color}}></div>
      </div>
    )}    
    </div>
    </div>
